    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2 rounded-full bg-purple-600 px-4 py-3 text-white shadow-lg shadow-purple-600/25 transition-all hover:bg-purple-500 hover:shadow-purple-500/30 hover:scale-105"
          title="AI Assistant"
        >
      ...
      <div
        className={cn(
          'fixed right-0 top-0 z-50 h-full w-[400px] transform border-l border-slate-700/50 bg-slate-900 shadow-2xl transition-transform duration-300 ease-in-out flex flex-col',
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-700/50 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">PMO AI Assistant</h3>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              ...
              <div
                className={cn(
                  ...
                  msg.role === 'user'
                    ...
                    : 'bg-slate-800 text-slate-200 border-l-2 border-purple-500 rounded-bl-sm'
                )}
              >
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="rounded-xl bg-slate-800 border-l-2 border-purple-500 px-4 py-3 rounded-bl-sm">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
            ...
            {QUICK_ACTIONS.map((action) => (
              <button
                ...
                disabled={isTyping}
                className="shrink-0 rounded-full border border-slate-700 bg-slate-800 px-3 py-1 text-xs text-slate-300 transition-colors hover:border-purple-500/50 hover:text-purple-300 disabled:opacity-50"
              >
                {action.label}
              </button>
            ))}
        ...
        <div className="border-t border-slate-700/50 px-4 py-3">
          ...
          >
            <input
              ...
              disabled={isTyping}
              className="flex-1 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:border-purple-500 focus:outline-none disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={!input.trim() || isTyping}
              className="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-600 text-white transition-colors hover:bg-purple-500 disabled:opacity-50 disabled:hover:bg-purple-600"
            >
