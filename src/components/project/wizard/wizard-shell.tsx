...
export function WizardShell({ children, onSubmit, validateStep }: WizardShellProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <div className="relative mb-6">
          <div className="h-20 w-20 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center animate-[scale-in_0.5s_ease-out]">
        ...
        <h2 className="text-2xl font-bold text-foreground mb-2">Project Registered Successfully!</h2>
        <p className="text-muted-foreground mb-1">{formData.name} has been created and is ready to go.</p>
        <p className="text-sm text-muted-foreground mb-8">Your project dashboard is being prepared.</p>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      {/* Progress bar */}
      <Progress value={progress} className="mb-8 h-1.5" indicatorClassName="bg-gradient-to-r from-violet-500 to-indigo-500" />
            ...
            <React.Fragment key={s.number}>
              <div className="flex flex-col items-center gap-1.5">
                <button
                  ...
                  className={cn(
                    ...
                    isCurrent && 'border-violet-500 bg-violet-500 text-white shadow-lg shadow-violet-500/25',
                    isCompleted && 'border-green-500 bg-green-500 text-white cursor-pointer hover:bg-green-600',
                    !isCurrent && !isCompleted && 'border-muted-foreground/30 text-muted-foreground/50 bg-background'
                  )}
                >
                  {isCompleted ? (
              ...
              {i < STEPS.length - 1 && (
                <div className={cn(
                  ...
                  s.number < currentStep ? 'bg-green-500' : 'bg-muted-foreground/20'
                )} />
              )}
            </React.Fragment>
          )
      ...
      <div className="flex justify-between border-t border-border pt-6">
        ...
        <div className="flex gap-3">
          {currentStep === STEPS.length ? (
            <Button onClick={handleSubmit} className="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white px-8">
              <svg className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
              Register Project
