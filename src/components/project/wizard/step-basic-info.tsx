...
export function StepBasicInfo({ formData, setFormData }: StepProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Category / Tags */}
      <div className="space-y-2">
        <label className="text-sm font-medium text-[var(--on-surface)]">Category / Tags</label>
        <div className="flex flex-wrap gap-2 mb-2">
          {formData.tags.map(tag => (
            <Badge key={tag} variant="secondary" className="pl-2.5 pr-1 py-1 gap-1 rounded-full bg-[var(--surface-container-high)] text-[var(--on-surface)]">
              {tag}
              <button
                type="button"
                onClick={() => removeTag(tag)}
                className="ml-1 h-4 w-4 rounded-full hover:bg-[var(--on-surface)]/10 flex items-center justify-center"
              >
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          <span className="text-xs text-[var(--outline)] mr-1 self-center">Suggestions:</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        {/* Sponsoring Division */}
        <div className="space-y-2">
          <label className="text-sm font-medium text-[var(--on-surface)]">Sponsoring Division</label>
          <Input
